/*=========================================================================
  ...
  Module:    vtkLeapBoxRepresentation.cxx
...
vtkStandardNewMacro(vtkLeapBoxRepresentation);

//----------------------------------------------------------------------------
vtkLeapBoxRepresentation::vtkLeapBoxRepresentation()
{
  // The initial state
  this->InteractionState = vtkLeapBoxRepresentation::Outside;
  ...
  //--> Makes sense just extra points represented by the handles
...
void vtkLeapBoxRepresentation::initLeapInfo(){
  this->TInfo1 = vtkTextActor::New();
  TInfo1->SetInput("LEAP OFF");
  TInfo1->SetPosition(10.0,10.0);
  TInfo1->GetTextProperty()->SetBold(1);
  TInfo1->GetTextProperty()->SetColor(0.0,0.0,1.0);
}

void vtkLeapBoxRepresentation::updateLeapInfo(){
  if(LeapInRange)
...
double vtkLeapBoxRepresentation::GetValue(){
  qDebug() << "Representation GetValue";

  return QDateTime::currentDateTime().toMSecsSinceEpoch()/1000.0;
}

void vtkLeapBoxRepresentation::SetValue(double val){
  qDebug() << "Representation Set Value: " << val;
}

void vtkLeapBoxRepresentation::SetBounds(double bnds[6]){
  qDebug() << "SetBounds";
}
...
//----------------------------------------------------------------------
void vtkLeapBoxRepresentation::GetPolyData(vtkPolyData *pd)
{
...
//----------------------------------------------------------------------
void vtkLeapBoxRepresentation::StartWidgetInteraction(double e[2])
{
...
//----------------------------------------------------------------------
void vtkLeapBoxRepresentation::WidgetInteraction(double e[2])
{
  ...
  // Process the motion
  if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF0 )
    {
    this->MoveMinusXFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF1 )
    {
    this->MovePlusXFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF2 )
    {
    this->MoveMinusYFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF3 )
    {
    this->MovePlusYFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF4 )
    {
    this->MoveMinusZFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::MoveF5 )
    {
    this->MovePlusZFace(prevPickPoint,pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::Translating )
    {
    this->Translate(prevPickPoint, pickPoint);
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::Scaling )
    {
    this->Scale(prevPickPoint, pickPoint,
                static_cast<int>(e[0]), static_cast<int>(e[1]));
    }

  else if ( this->InteractionState == vtkLeapBoxRepresentation::Rotating )
    {
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MoveFace(double *p1, double *p2, double *dir,
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::GetDirection(const double Nx[3],const double Ny[3],
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MovePlusXFace(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MoveMinusXFace(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MovePlusYFace(double *p1, double *p2)
{
  double *pts =
     static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MoveMinusYFace(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MovePlusZFace(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::MoveMinusZFace(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
...
void vtkLeapBoxRepresentation::Translate(double *p1, double *p2)
{
  double *pts =
    static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
  double v[3];
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::Scale(double *vtkNotUsed(p1),
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::ComputeNormals()
{
  double *pts =
     static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
  double *p0 = pts;
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::GetPlanes(vtkPlanes *planes)
{
  if ( ! planes )
    {
    return;
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::Rotate(int X,
                                  int Y,
                                  double *p1,
                                  double *p2,
                                  double *vpn)
{
  ...
  //What its doing here is moving the object back to origin then applying the
  //rotation and subsequently moving it back it origin position
  this->Transform->Identity();
  this->Transform->Translate(center[0],center[1],center[2]);
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::CreateDefaultProperties()
{
  // Handle properties
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::PlaceWidget(double bds[6])
{
  qDebug() << "PlaceWidget Yea";
  int i;
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::SetTransform(vtkTransform* t)
{
  if (!t)
    {
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::SetOutlineFaceWires(int newValue)
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::SetOutlineCursorWires(int newValue)
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::GenerateOutline()
{
...
//----------------------------------------------------------------------------
int vtkLeapBoxRepresentation::ComputeInteractionState(int X, int Y, int modify)
{
  ...
  if ( path != NULL )
    ...
    if ( this->CurrentHandle == this->Handle[0] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF0;
      }
    else if ( this->CurrentHandle == this->Handle[1] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF1;
      }
    else if ( this->CurrentHandle == this->Handle[2] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF2;
      }
    else if ( this->CurrentHandle == this->Handle[3] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF3;
      }
    else if ( this->CurrentHandle == this->Handle[4] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF4;
      }
    else if ( this->CurrentHandle == this->Handle[5] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::MoveF5;
      }
    else if ( this->CurrentHandle == this->Handle[6] )
      {
      this->InteractionState = vtkLeapBoxRepresentation::Translating;
      }
    }
    ...
    if ( path != NULL )
      ...
      if ( !modify )
        {
        this->InteractionState = vtkLeapBoxRepresentation::Rotating;
        }
      else
        {
...
//----------------------------------------------------------------------
void vtkLeapBoxRepresentation::SetInteractionState(int state)
{
  // Clamp to allowable values
  state = ( state < vtkLeapBoxRepresentation::Outside ? vtkLeapBoxRepresentation::Outside :
            (state > vtkLeapBoxRepresentation::Scaling ? vtkLeapBoxRepresentation::Scaling : state) );
  ...
  switch (state)
    ...
    case vtkLeapBoxRepresentation::MoveF5:
      ...
      break;
    case vtkLeapBoxRepresentation::Rotating:
      this->HighlightOutline(0);
      this->HighlightHandle(NULL);
      this->HighlightFace(this->HexPicker->GetCellId());
      break;
    case vtkLeapBoxRepresentation::Translating:
    case vtkLeapBoxRepresentation::Scaling:
...
//----------------------------------------------------------------------
double *vtkLeapBoxRepresentation::GetBounds()
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::BuildRepresentation()
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::ReleaseGraphicsResources(vtkWindow *w)
{
  this->Renderer->RemoveActor(this->TInfo1);
...
//----------------------------------------------------------------------------
int vtkLeapBoxRepresentation::RenderOpaqueGeometry(vtkViewport *v)
{
  int count=0;
  this->BuildRepresentation();
  this->Renderer->AddActor2D(this->TInfo1);
  count += 1;
...
//----------------------------------------------------------------------------
int vtkLeapBoxRepresentation::RenderTranslucentPolygonalGeometry(vtkViewport *v)
{
  int count=0;
  this->BuildRepresentation();
  this->Renderer->AddActor2D(this->TInfo1);
...
//----------------------------------------------------------------------------
int vtkLeapBoxRepresentation::HasTranslucentPolygonalGeometry()
{
  int result=0;
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::PositionHandles()
{
  double *pts =
     static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(0);
  double *p0 = pts;
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::HandlesOn()
{
  for (int i=0; i<7; i++)
    {
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::HandlesOff()
{
  for (int i=0; i<7; i++)
    {
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::SizeHandles()
{
  double *center
    = static_cast<vtkDoubleArray *>(this->Points->GetData())->GetPointer(3*14);
  double radius =
...
//----------------------------------------------------------------------------
int vtkLeapBoxRepresentation::HighlightHandle(vtkProp *prop)
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::HighlightFace(int cellId)
{
  if ( cellId >= 0 )
    {
    curFace = cellId;
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::HighlightOutline(int highlight)
{
  if ( highlight )
    {
...
//------------------------------------------------------------------------------
void vtkLeapBoxRepresentation::RegisterPickers()
{
...
//----------------------------------------------------------------------------
void vtkLeapBoxRepresentation::PrintSelf(ostream& os, vtkIndent indent)
{
